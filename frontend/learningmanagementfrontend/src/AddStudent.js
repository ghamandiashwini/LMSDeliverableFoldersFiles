...
function AddStudent() {
 ...
 /* const handleSearchUser = async () => {
  ...
  const handleCreateNewStudent = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/student/createnewstudent', { rollno, firstname,lastname , location, userid });
  ...
  return (
    <div> 
     ...
     <button onClick={handleCreateNewStudent}>Add</button>
      <p>{message}</p>
    </div>
  );
