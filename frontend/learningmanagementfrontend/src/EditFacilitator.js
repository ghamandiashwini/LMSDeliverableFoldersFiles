...
function EditFacilitator() {
  ...
  const handleSearch = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/facilitator/searchfacilitator/${searchval}`);
 ...
 const handleDeleteFaclilitator = async (id) => {
  try {
      const response = await fetch(`http://localhost:5000/api/facilitator/delete/${searchresult._id}`, {
          method: "DELETE",
      });
      setMessage(response.data.message);
      /*if (response.ok) {
  ...
  return (
    <div>
       ...
       <div className="col-md-6">
                    <div className="card">
                        <div className="card-header">
                            <b>Search Facilitator</b>
                        </div>
                        <div className="card-body">
                            <div className="row">
                                <div className="col-md-4">
                                <label>Facilitator No</label>
                                </div>
                                <div className="col-md-4">
                                <input type="text"  name="txtfno"  ref={enterdcourseno}  onChange={(e)=>{setSearchval(e.target.value)}}  className="form-control" />
                                </div>
                                <div className="col-md-4">
     ...
     <button onClick={handleDeleteFaclilitator}>Delete</button>                         
     <p>{message}</p>
